...
# Requirements: apt-get install jpeginfo screen wget jq ffmpeg uuid sudo imagemagick
...
if [[ $1 == 'start' ]]; then
sudo -u root screen -ls darknetbg | grep -E '\s+[0-9]+\.' | awk -F ' ' '{print $1}' | while read x; do screen -XS $x quit; done

# Start the watchdog (split multi-line)
if ! screen -list | grep -q "darknetwatchdog"; then
echo $(date +%T) "Starting darknetwatchdog..."
sudo -u root screen -S darknetwatchdog -d -m bash -c "while true; do sleep 60; if pgrep -fl $darknetpath/darknet | grep -Evq 'screen|bash'; then echo \$(date +%T) 'Darknet is already running'; \
else echo \$(date +%T) 'Watchdog is attempting to restart Darknet...' ; sudo -u root screen -S darknetrestart -d -m timeout 55 $darknetpath/main.sh 'start'; fi; \
...
find /tmp/darknet/*.jpeg -mmin +5 -type f -delete 2>&1 ; if (echo $productionmode | grep -q 'yes'); then if (find /tmp/darknet/darknetoutput -mmin +3 -type f | grep -q '/tmp/darknet/darknetoutput'); then echo \$(date +%T) 'Darknet has not processed data within timeout attempting to restart...' ; sudo -u root screen -S darknetrestart -d -m timeout 55 $darknetpath/main.sh 'start' ; fi; fi; done"
fi

mkdir -p "/tmp/darknet"
if ! grep -qs /tmp/darknet /proc/mounts; then mount -t tmpfs -o size=512m tmpfs /tmp/darknet >/dev/null 2>&1; fi
...
sudo -u root screen -ls darknetbg | grep -E '\s+[0-9]+\.' | awk -F ' ' '{print $1}' | while read x; do screen -XS $x quit; done
sudo -u root screen -ls darknetwatchdog | grep -E '\s+[0-9]+\.' | awk -F ' ' '{print $1}' | while read x; do screen -XS $x quit; done
sudo -u root screen -ls darknetrestart | grep -E '\s+[0-9]+\.' | awk -F ' ' '{print $1}' | while read x; do screen -XS $x quit; done
sudo -u root screen -ls darknetstartup | grep -E '\s+[0-9]+\.' | awk -F ' ' '{print $1}' | while read x; do screen -XS $x quit; done
fi
...
mkdir -p /tmp/darknet
cd $darknetpath
bash -c "export PATH=/usr/local/cuda-10.1/bin${PATH:+:${PATH}} ; \
...
elif [[ $2 == "local" ]]; then # Get image via local file
mv "$3" "/tmp/darknet/source-$uuid.jpeg"
...
elif [[ $2 == "blueiris" ]]; then # Get image from Blue Iris
sessionid=$(curl -s --max-time 60 -k -d '{"cmd":"login"}' -H "Content-Type: application/json" -X POST "$3/json" | jq -r .session)
formatforlogin=$(echo -n "$4:$sessionid:$5" | md5sum | awk '{ print $1 }')
blueirislogin=$(curl -s --max-time 60 -k -d '{"cmd":"login","session":"'"$sessionid"'","response":"'"$formatforlogin"'"}' -H "Content-Type: application/json" -X POST "$3/json")
wget -q --timeout=5 --tries=1 --no-check-certificate -O "/tmp/darknet/source-$uuid.jpeg" "$3/image/$6?q=100&s=100&session=$sessionid"
fi
