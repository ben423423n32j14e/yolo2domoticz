...
find /tmp/darknet/*.jpeg -mmin +5 -type f -delete ; if (echo $productionmode | grep -q 'yes'); then if (find /tmp/darknet/darknetoutput -mmin +3 -type f | grep -q '/tmp/darknet/darknetoutput'); then echo \$(date +%T) 'Darknet has not processed data within timeout attempting to restart...' ; sudo -u root /usr/bin/screen -S darknetrestart -d -m timeout 55 $darknetpath/main.sh 'start' ; fi; fi; done"
fi
